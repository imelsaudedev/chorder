...
export function parseChordPro(chordproString: string) {
...
export function toHalfToneIndex(index: number) {
  const n = HALF_TONES.length;
  return ((index % n) + n) % n;
}

export function keyFromChord(chord: string): SongKey | null {
  const re = /([ABCDEFG][b#]*)\.*/;
...
export function getKeyFromChords(chords: string[]) {
  const keys = chords
...
export function harmonicIndex(key: SongKey) {
  switch (key) {
    case "C":
    case "B#":
      return 0;
