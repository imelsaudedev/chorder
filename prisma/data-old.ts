...
export async function retrieveService(slug: string) {
  ...
  }) as Promise<ServiceWithUnits | null>;
}
...
export async function createOrUpdateService(
  ...
  date: Date,
  units: (Omit<ServiceUnit, "id" | "serviceId"> & {
    arrangement?: SongArrangement & { units: ClientSongUnit[] };
  } & {
    id?: number;
    serviceId?: number;
  })[]
  ...
  const serviceUnits = await Promise.all(unitPromises);
  ...
  return prisma.service.upsert({
      ...
      units: {
        ...
        createMany: {
          data: serviceUnits.map((unit) => ({
            ...
            arrangementName: unit.arrangement!!.name,
            key: unit.arrangement!!.key,
            order: unit.order,
          })),
        },
    ...
    create: {
      ...
      units: {
        createMany: {
          data: serviceUnits.map((unit, order) => ({
            type: unit.type,
            arrangementId: unit.arrangementId,
            semitoneTranspose: unit.semitoneTranspose,
            arrangementName: unit.arrangement!!.name,
