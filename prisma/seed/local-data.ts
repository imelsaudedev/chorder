import { SongUnitType, PrismaClient } from '@/generated/prisma'
...
export async function createSongsFromLocal(prisma: PrismaClient) {
  ...
  for (const song of songData) {
    const existingSong = song.legacyId ? await prisma.song.findUnique({
    ...
    if (existingSong) {
      ...
      const newSong = await prisma.song.create({ data: song, include: { arrangements: { include: {units: true}} } });
      songs.push(newSong);
    }
  }
  return songs;
...
export async function createServicesFromLocal(prisma: PrismaClient, songs: SongWithArrangements[]) {
  const serviceData = getServiceData(songs);

  for (const service of serviceData) {
    const existingService = service.legacyId ? await prisma.service.findUnique({
...
function getServiceData(songs: SongWithArrangements[]) {
  const songDataBySlug = Object.fromEntries(songs.map(song => [song.slug, song]));
...
return baseServices.map(serviceData => ({
  ...
  units: {
    create: serviceData.units.map((unit, idx) => {
      ...
      const unitsByInternalId = Object.fromEntries(arrangement.units.map(unit => [unit.order, unit]));
