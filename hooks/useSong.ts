...
export default function useSong(song: Song, arrangementIndex: number): SongHook {
  const { arrangement, isNewArrangement } = useMemo(() => {
  ...
  const { units, songMap, songUnitMap, createUnit, addUnit, moveUnitUp, moveUnitDown, buildRemoveUnitHandler, buildUpdateUnitHandler } = useUnits(arrangement.units, arrangement.songMap, nextUnitId);

  const builtSong = useMemo(
    () => buildSong(arrangementIndex, title, artist, song.slug, key, units, songMap, lastUnitId, song.arrangements),
    [arrangementIndex, title, artist, song.slug, key, units, songMap, lastUnitId, song.arrangements]
  );

  return { song: builtSong, arrangementIndex, isNewArrangement, title, setTitle, artist, setArtist, songKey: key, setSongKey: setKey, units, songMap, songUnitMap, createUnit, addUnit, moveUnitUp, moveUnitDown, buildRemoveUnitHandler, buildUpdateUnitHandler };
}

function buildSong(arrangementIndex: number, title: string, artist: string | null, slug: string | undefined, key: string | undefined, units: SongUnit[], songMap: number[], lastUnitId: number, arrangements: SongArrangement[]) {
...
function useUnits(units: SongUnit[], songMap: number[], nextUnitId: () => number) {
  ...
  const songUnitMap = useMemo(() => internalSongMap.map(unitId => internalUnits.find(unit => unit.internalId === unitId)).filter((unit) => !!unit) as SongUnit[], [internalUnits, internalSongMap]);
  ...
  const addUnit = useCallback((unit: SongUnit) => {
    if (!internalUnits.find(otherUnit => otherUnit.internalId === unit.internalId)) throw new Error(`Unit ${unit.internalId} not found`);
    setInternalSongMap(songMap => [...songMap, unit.internalId]);
  }, [internalUnits]);

  const moveUnitUp = useCallback((unitIndex: number) => {
  ...
  const moveUnitDown = useCallback((unitIndex: number) => {
  ...
  const buildRemoveUnitHandler = useCallback((unitIndex: number) => {
    return () => {
      const newSongMap = [...internalSongMap];
  ...
  const buildUpdateUnitHandler = useCallback((unitIndex: number) => {
  ...
  }, [internalUnits, songUnitMap]);

  return { units: internalUnits, songMap: internalSongMap, songUnitMap, createUnit, addUnit, moveUnitUp, moveUnitDown, buildRemoveUnitHandler, buildUpdateUnitHandler };
}
