...
export function getLyrics(chordproString: string) {
  try {
    ...
    return parsedSong.lines
      .map((line) => line.items.map((item: any) => (item._name === 'comment' ? '' : item.lyrics)).join(''))
      .join('\n');
  } catch (e) {
    console.log(e);
    return null;
...
export function getChords(chordproString?: string) {
  ...
  try {
    const parsedSong = parseChordPro(chordproString);
    return parsedSong.lines
      .map((line) => line.items.map((item: any) => (item._name === 'comment' ? '' : item.chords)))
      .flat(2);
  } catch (e) {
    console.log(e);
    return [];
...
export const getKeyFromChords = (chords: string[]) => {
  const keys = chords.map((chord) => keyFromChord(chord)).filter((key) => key !== null);
...
function getMostLikelyKey(chords: string[]) {
  let key = 'C';
  let maxCount = 0;
  let earliestOccurrence = Number.MAX_SAFE_INTEGER;
  for (const k in KEYS) {
    const count = chords.filter((chord) => KEYS[k as keyof typeof KEYS].includes(chord)).length;
    const occurrence = chords.findIndex((chord) => chord === k);
    if (count > maxCount) {
      maxCount = count;
      key = k;
      if (occurrence >= 0) {
    ...
    } else if (count === maxCount && occurrence >= 0 && occurrence < earliestOccurrence) {
      earliestOccurrence = occurrence;
      key = k;
    }
  }
...
export const transposeChord = (chord: string, originalKey: string, semitones: number) => {
  ...
  if (slashIdx >= 0) {
    transposed =
      ...
      transposeChord(transposed.substring(slashIdx + 1), originalKey, semitones);
  }
  return transposed;
};
