...
export type Comment = { _name: "comment"; _value: string };
export type ChordProItem = Comment | ChordLyricsPair;
...
export function getLyrics(chordproString: string) {
  try {
    ...
    return parsedSong.lines
      .map((line) =>
        (line.items as ChordProItem[])
          .map((item: ChordProItem) =>
            item._name === "comment" ? "" : (item as ChordLyricsPair).lyrics
          )
          .join("")
      )
      .join("\n");
...
export function getChords(chordproString?: string) {
  ...
  try {
    ...
    return parsedSong.lines
      .map((line) =>
        (line.items as ChordProItem[]).map((item: ChordProItem) =>
          item._name === "comment" ? "" : (item as ChordLyricsPair).chords
        )
      )
      .flat(2);
  } catch (e) {
